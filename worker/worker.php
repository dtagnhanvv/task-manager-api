...
$concurrentTubeName = $container->getParameter('biddy.worker.concurrent_tube_name');
$lockKeyPrefix = $container->getParameter('biddy.worker.lock_key_prefix');

$pid = getmypid();
$requestStop = false;

$appShutdown = function () use (&$requestStop, $pid, &$logger) {
    $logger->notice(sprintf("Core PID %d has received a request to stop gracefully", $pid));
...
$redis = $container->get('biddy.redis.app_cache');

$redLock = new RedLock([$redis]);

$concurrentJobScheduler = $container->get('biddy.worker.scheduler.concurrent_job_scheduler');
$linearJobScheduler = $container->get('biddy.worker.scheduler.linear_job_scheduler');

// this worker pool is not watched by this main worker process
$linearWorkerPool = $container->get('biddy.worker.job.linear.worker_pool');

$concurrentWorkerPool = $container->get('biddy.worker.job.concurrent.worker_pool');
...
while (1) {
    ...
    try {
        $jobParams = new \Biddy\Worker\Core\JobParams(json_decode($rawJobData, true));
        ...
        if ($jobWorker instanceof \Biddy\Worker\Core\Job\LockableJobInterface) {
