...
abstract class EntityVoterAbstract implements VoterInterface
{
    ...
    public function vote(TokenInterface $token, $entity, array $attributes)
    {
        ...
        // allow admins and only allow accounts who own the site entity

        if ($user->hasRole('ROLE_ADMIN')) {
            return VoterInterface::ACCESS_GRANTED;
        }

        if ($user->hasRole('ROLE_SALE')) {
            return VoterInterface::ACCESS_GRANTED;
        }

        if ($user->hasRole('ROLE_ACCOUNT') && $this->isAccountActionAllowed($entity, $user, $attribute)) {
            return VoterInterface::ACCESS_GRANTED;
        }
    ...
    /**
     * Checks to see if a account has permission to perform an action
     *
     * @param ModelInterface $entity
     ...
     * @return bool
     */
    abstract protected function isAccountActionAllowed($entity, UserEntityInterface $user, $action);
}